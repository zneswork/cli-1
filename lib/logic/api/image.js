...
const filterByAnnotations = (images, {filter, type })=>{
  kefir.sequentially(100, images).map(()=>{
...
const annotateImage = async (dockerImageId, annotations) => {
  ...
  const getImageAnnotations = async (dockerImageId)=>{
    console.log(`image id ${dockerImageId}`);
  const options = {
      url: `/api/images/${encodeURIComponent(dockerImageId)}/metadata`,
      method: 'GET',
